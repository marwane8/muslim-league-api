...
def get_team_info(season_id: int = Path(None,description="ID of a Season")):
    return db_acc.get_teams_data(season_id) 

get_teams_standings_summary= "Returns a the team standings of a season"
@router.get("/teams/{season_id}/{sport}" ,summary=get_teams_summary, response_model=list[BballStandings])
def get_team_standings(sport: str = Path("basketball"),season_id: int = Path(None,description="ID of a Season")):
    return proc.get_standings(season_id) 
...
get_game_summary= "Returns a list of games for a given date"
@router.get("/games/{date}" ,summary=get_game_summary, response_model=list[Game])
def get_games_by_date(date: int = Path(None,description="The ID of a Season")):
    return db_acc.get_games_by_date(date)
